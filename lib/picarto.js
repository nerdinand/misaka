...
Room.prototype.connect = function() {
  ...
  this.initMessagesCallbacks();
  this.initCallbacks(['users', 'mods', 'admin', 'clear', 'bannedUsers']);
...
Room.prototype.onMessage = function(callback) {
  return this.on('messages', 'child_added', callback);
};

Room.prototype.onUserJoin = function(callback) {
  return this.on('users', 'child_added', callback);
};

Room.prototype.onUserLeave = function(callback) {
  return this.on('users', 'child_removed', callback);
};

Room.prototype.onMod = function(callback) {
  return this.on('mods', 'child_added', callback);
};

Room.prototype.onAdmin = function(callback) {
  return this.on('admin', 'child_added', callback);
};
...
Room.prototype.initMessagesCallbacks = function() {
  var base = this.firebase.messages;
  var room = this;

  var cb = this.callbacks['messages'];

  // Only listen for 1 message, otherwise we get
  // the whole history
  this.firstMessage = false;
  base.limit(1).on('child_added', function(e) {
    // We don't care about the first message, it's part of the chat history
  ...
  base.on('child_changed', function(e) {
    room.fireCallbacks(cb.child_changed, e);
  });

  base.on('child_removed', function(e) {
