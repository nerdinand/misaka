...
/**
 * Get the rawr points row for a user in a channel. If no row found, will insert
 * a default row and return it in the callback.
 * @param {String} channel Channel name
 * @param {String} username Username
 */
DbManager.prototype.getRawrPointsRow = function(channel, username, callback) {
  if(!this.checkInitialized(callback)) {
    return;
  }

  var getRawrPointsRow = DbManager.prototype.getRawrPointsRow.bind(this),
      db = this._db,
      table = this._tables.rawr_points,
      query = table.select().from(table)
                .where(table.channel.equals(channel)).and(table.user.equals(username))
    ...
    if(!err) {
      if(rows.length > 0) {
        ...
        db.run(query, function(err) {
          if(!err) {
            // Recursive call now that the row has been inserted
            getRawrPointsRow(channel, username, callback);
          } else if(callback) {
            callback(err);
          }
        });
...
/**
 * Increment a user's RAWR subpoints, and if it hits the subpoint roof, award them
 * a point and reset the subpoint count to 0.
 * @param {String} channel Channel name
 * @param {String} username Username
 * @param {Number} roof Subpoint roof
 */
DbManager.prototype.incrementRawrSubpoint = function(channel, username, roof, callback) {
  if(!this.checkInitialized(callback)) {
    return;
  }
  ...
  this.getRawrPointsRow(channel, username, function(err, row) {
    if(!err) {
      var query, hitRoof = false,
          subpoints = row.subpoints + 1;

      if(subpoints === roof) {
        hitRoof = true;
        query = table
                  .update({ points: (row.points + 1), subpoints: 0 })
                  .where(table.channel.equals(channel)).and(table.user.equals(username))
                  .toString();
      } else {
        hitRoof = false;
        query = table
                  .update({ subpoints: subpoints })
                  .where(table.channel.equals(channel)).and(table.user.equals(username))
                  .toString();
      }
  ...
  this._tables = {
    ...
    rawr_points: sqlite.define({
      ...
      columns: [
        ...
        { name: 'points', dataType: 'integer DEFAULT 0', notNull: true },
        { name: 'subpoints', dataType: 'integer DEFAULT 0', notNull: true },
