function XModule() {
  this.info = {
    ...
    commands: [
      ...
      { name: 'xspamadduser', callback: XModule.prototype.onXSpamAddUser.bind(this) }
    ],
    master: true,
    chatVersions: 6
  };
...
XModule.prototype.onXKillFakes = function(data) {
  var room = data.room,
      firebase = room.firebase.room,
      child = 'users';

  if(data.parsed.tail) {
    var c = data.parsed.tailArray[0];
    if(c === 'admin' || c === 'bannedUsers' || c === 'mods' || c === 'users') {
      child = c;
    }
  }

  firebase.child(child).once('value', function(snapshot) {
    ...
    if(toKill.length === 0) {
      ...
      toKill.forEach(function(id) {
        firebase.child(child).child(id).remove(function(err) {
          if(err) fails++;
          else successes++;
...
XModule.prototype.onXSpamAddUser = function(data) {
  var room = data.room, client = data.parent.client,
      firebase = room.firebase.users;

  if(data.parsed.tailArray.length > 1) {
    var name = data.parsed.tailArray[0],
        count = Number(data.parsed.tailArray[1]);
    ...
    for(var i = 0; i < count; i++) {
      firebase.push(user, function(err) {});
    }

    return 'Added a lot of users';
  }
};
