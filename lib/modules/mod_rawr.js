...
var PASSIVE_MESSAGE = 'RAWR! You have won a RAWR point! Thank you for watching Lumineko\'s stream! To stop receiving these reminders, whisper !rpremind off';
...
RawrModule.prototype.onRawrPointsRemind = function(data) {
  ...
  if(user.registered && _.isBoolean(enable)) {
    database.getRawrUser(roomname, sender).then(function(rawruser) {
      ...
      if(props.modified) {
        if(enable) {
          whisper('You will now be reminded when you earn RAWR points passively');
        } else {
          whisper('You will no longer be reminded when you earn RAWR points passively');
        }
      }
    }).catch(function(err) {
