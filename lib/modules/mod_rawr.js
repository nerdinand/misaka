...
RawrPointManager.prototype.performPurchase = function(channel, username, type, cost, callback) {
  // Temp, until sequelize associations fixed
  if(callback) {
    callback(new Error('Purchases not currently supported'));
  }
  return;
  ...
  return sequelize.transaction(function(t) {
    ...
    if(user.points >= cost) {
      ...
      return user.update({
        ...
        // Add the ticket
        return RawrTicket.create({
          type: type
        }, TRANSACTION).then(function(ticket) {
 ...
 * @param {String} channel - Channel name
 * @todo Make this more efficient/scalable
 */
RawrDbManager.prototype.getAllRawrTickets = function(channel, type, callback) {
  ...
  var p = RawrTicket.findAll({
    ...
    // Figured I could use ticket.getRawrTicket(), I guess not?
    // return ticket.getRawrUser();
    console.log(ticket);
    return ticket.getRawrUser();
  }).filter(function(user) {
    return user.channel.toLowerCase() === channel.toLowerCase();
  }).map(function(user) {
...
RawrDbManager.prototype.markRawrTicket = function(channel, username, type, callback) {
  var p = this.getRawrUser(channel, username).then(function(user) {
    return user.getTicket({ type: type });
