...
function PlugDJModule() {
  ...
  this.info = {
    ...
    commands: [
      ...
      { name: 'skip', callback: PlugDJModule.prototype.onSkip.bind(this) },
      ...
      { name: 'video', callback: PlugDJModule.prototype.onSong.bind(this) },
      { name: 'videourl', callback: PlugDJModule.prototype.onSongUrl.bind(this) }
    ],
    callbacks: {
...
PlugDJModule.prototype.onSkip = function(data) {
  var user = data.user,
      logger = this.logger,
      plugapi = this.plugapi;

  if((user.admin || user.mod) && plugapi) {
    plugapi.moderateForceSkip(function(success) {
      logger.log('debug', 'plugAPI moderateForceSkip', { success: success });
    });
  }
};
...
PlugDJModule.prototype.onJoin = function(data) {
  this.send = data.send;
  var logger = this.logger = data.logger;

  if(!data.config || data.config.email === undefined
      || data.config.password === undefined) {
    logger.warn('E-mail and password needed for plug.dj module');
    return;
  }
...
PlugDJModule.prototype.initClient = function() {
  ...
  }, function(err, plugapi) {
    if(!err) {
      module.plugapi = plugapi;

      plugapi.on('roomJoin', function(room) {
        logger.info('Joined plug.dj room', { room: room });
      });
      ...
      var reconn = function() {
        logger.info('Reconnecting to plug.dj room...');
      ...
      if(module.room !== undefined) {
        logger.info('Connecting to room on plug.dj...', { room: module.room });
        plugapi.connect(module.room);
      }
    } else {
      logger.error(error, { msg: 'Error while using PlugAPI' });
    }
  });
};
