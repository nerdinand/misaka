    ...
    if(callback) {
      if(!err) {
        ...
        if(meta['x-ratelimit-limit'] === '60') {
          console.warn('Github auth not active or failed');
        }
...
GithubModule.prototype.getCommitString = function(commit) {
  ...
  var author = commit.committer.login,
      ...
      str = ('New commit by ' + author + ': ' + message + ' (sha: ' + sha + ')');
  return str;
};
    ...
    module.fetchLatestCommit(function(error, commit) {
      if(!error) {
        ...
        if(module.current.sha !== commit.sha) {
          ...
          module.send(module.getCommitString());
        }
      } else {
        console.warn('Error fetching latest commit:', commit);
      }
    });
  }, this.wait);
};
...
GithubModule.prototype.onJoin = function(data) {
  ...
  // Token is required
  if(!this.token) {
    console.warn('No oauth token given for Github module');
    return;
  }
  ...
  this.fetchLatestCommit(function(error, commit) {
    if(!error) {
      ...
      console.warn('Error fetching current commit:', error);
    }
  });
};

GithubModule.prototype.onCommit = function(data) {
  return this.getCommitString(this.current);
};
