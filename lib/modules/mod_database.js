...
function DatabaseModule() {
  this.info = {
    ...
    commands: [
      ...
      { name: 'msgcount', callback: DatabaseModule.prototype.onMsgCount.bind(this) },
      { name: 'seen', callback: DatabaseModule.prototype.onSeen.bind(this) }
    ]
  };
}

DatabaseModule.prototype.onMsgCount = function(data) {
  var roomname = data.room.name,
      db = data.database,
      logger = data.logger,
      send = data.send;

  db.getMessageLogCount(roomname, '-1 day', function(err, count) {
    if(!err) {
      send(count + ' messages logged in the past day.');
    } else {
      logger.error(err);
    }
  });
  ...
  if(data.parsed.tail) {
    ...
    db.getLastLogEntry(roomname, username, function(err, row) {
      if(!err && row) {
        ...
        send(row.user + ' was last seen: ' + timeStr + ' ago.');
      } else {
        if(err) {
          logger.error(err);
        }
